...
function smoothScrollToPosition(targetY, duration = 950) {
...
function jumpTo(sectionId) {
    ...
    document.body.classList.toggle('services-active', sectionId === 'products' || isLoggedIn);
    document.body.classList.toggle('about-active', sectionId === 'about');
    ...
    if (sectionId === 'home') {
        ...
        if (mainHeader && window.scrollY < 50) mainHeader.classList.remove('scrolled');
    } 
    else {
    ...
    if (target) {
        ...
        if (sectionId === 'home') {
            ...
            const offset = window.innerWidth <= 560 ? 178 : (window.innerWidth <= 900 ? 142 : 82);
            smoothScrollToPosition(Math.max(0, target.offsetTop - offset), 1050);
        }
...
window.addEventListener('scroll', function() {
    ...
    document.body.classList.toggle('services-active', currentSectionId === 'products' || isLoggedIn);
    document.body.classList.toggle('about-active', currentSectionId === 'about');
    document.body.classList.toggle('contact-active', currentSectionId === 'contact');
});
...
function runNavSearch() {
    ...
    const sectionMatches = [
        { keywords: ['home', 'printify'], target: 'home' },
        { keywords: ['about', 'about us', 'company', 'mission', 'vision'], target: 'about' },
        { keywords: ['services', 'service', 'printing', 'document', 'photo', 'photocopy', 'scanning', 'lamination', 'binding'], target: 'products' },
        { keywords: ['contact', 'support', 'email', 'phone', 'message'], target: 'contact' }
    ];

    const match = sectionMatches.find(item => item.keywords.some(keyword => keyword.includes(query) || query.includes(keyword)));
    if (match) {
        jumpTo(match.target);
        return;
    }
...
function backToMain() {
    ...
    if (servicesSection) {
        ...
        smoothScrollToPosition(Math.max(0, servicesSection.offsetTop - 82), 950);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.section:not(#home) > *').forEach((element) => {
    ...
    if (searchBtn) searchBtn.addEventListener('click', runNavSearch);
    if (searchInput) {
    ...
    document.querySelectorAll('.scroll-reveal').forEach(s => observer.observe(s));

    const initialSection = (window.location.hash || '#home').replace('#', '');
    if (document.getElementById(initialSection)) {
        setTimeout(() => jumpTo(initialSection), 60);
    } else {
